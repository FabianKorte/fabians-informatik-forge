...
serve(async (req) => {
  ...
  try {
    ...
    if (messages.length === 0 || messages.length > 50) {
      return new Response(JSON.stringify({ error: "Messages array must contain 1-50 messages" }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    for (const msg of messages) {
      ...
      // Limit message content length (max 5000 characters per message)
      if (typeof msg.content !== 'string' || msg.content.length > 5000) {
        return new Response(JSON.stringify({ error: "Message content must be a string with max 5000 characters" }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
