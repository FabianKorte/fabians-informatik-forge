  ...
  datenbanken: [
    {
      ...
      cards: [
        { front: "Relationale Datenbank - Grundkonzept", back: "**Definition:** Daten werden in Tabellen (Relationen) mit Zeilen (Tupel) und Spalten (Attribute) gespeichert. **IHK-Kernwissen:** ACID-Eigenschaften (Atomicity, Consistency, Isolation, Durability), Normalisierung zur Vermeidung von Redundanzen, SQL als Abfragesprache. **Vorteile:** Datenintegrität, Konsistenz, strukturierte Abfragen. **Beispiel:** Kunden-Tabelle mit Attributen: ID, Name, Adresse, Telefon." },
        { front: "Primärschlüssel (Primary Key)", back: "**Definition:** Eindeutige Identifikation jeder Tabellenzeile. **IHK-Eigenschaften:** NOT NULL, UNIQUE, unveränderlich. **Arten:** Einfacher Schlüssel (eine Spalte) oder zusammengesetzter Schlüssel (mehrere Spalten). **Automatische Funktionen:** Index-Erstellung, Referenz für Fremdschlüssel. **Praxisbeispiel:** Kunden-ID als AUTO_INCREMENT INTEGER oder UUID. **Prüfungstipp:** Kann nicht NULL sein und muss eindeutig bleiben!" },
        { front: "Fremdschlüssel (Foreign Key)", back: "**Definition:** Verweis auf Primärschlüssel einer anderen Tabelle zur Herstellung von Beziehungen. **IHK-Funktionen:** Referenzielle Integrität, Cascade-Optionen (ON DELETE CASCADE/SET NULL/RESTRICT). **Bedeutung:** Ermöglicht Joins zwischen Tabellen, verhindert inkonsistente Daten. **Beispiel:** Bestellung.KundenID verweist auf Kunde.ID. **Prüfungsaspekt:** Kann NULL sein (optionale Beziehung), aber wenn Wert vorhanden, muss er in Referenz-Tabelle existieren." },
        { front: "1. Normalform (1NF) - Atomarität", back: "**Definition:** Jede Zelle enthält nur einen atomaren (unteilbaren) Wert, keine wiederholenden Gruppen. **IHK-Beispiel:** Statt 'Tel1: 123, Tel2: 456' → separate Zeilen oder Tabelle für Telefonnummern. **Verletzung:** Arrays in Zellen, kommaseparierte Listen. **Ziel:** Eliminierung mehrwertiger Attribute. **Praxisumsetzung:** Normalisierung durch Aufteilung in separate Entitäten mit 1:n-Beziehungen." },
        { front: "2. Normalform (2NF) - Vollständige Abhängigkeit", back: "**Voraussetzung:** Erfüllt 1NF. **Definition:** Alle Nicht-Schlüssel-Attribute sind vollständig funktional abhängig vom gesamten Primärschlüssel. **IHK-Problem:** Partielle Abhängigkeiten bei zusammengesetzten Schlüsseln. **Beispiel:** Bei Schlüssel (BestellID, ProduktID) darf Produktname nicht nur von ProduktID abhängen. **Lösung:** Aufteilung in separate Tabellen. **Prüfungstrick:** Nur bei zusammengesetzten Schlüsseln relevant!" },
        { front: "3. Normalform (3NF) - Transitive Abhängigkeiten", back: "**Voraussetzung:** Erfüllt 2NF. **Definition:** Keine transitiven Abhängigkeiten zwischen Nicht-Schlüssel-Attributen. **IHK-Beispiel:** Kunde → Stadt → PLZ ist transitiv - PLZ sollte nicht direkt in Kunden-Tabelle. **Problem:** A → B → C (indirekter Zusammenhang). **Lösung:** Separate Tabellen für transitive Abhängigkeiten. **Vorteil:** Reduziert Update-Anomalien, spart Speicherplatz, verhindert Inkonsistenzen." },
        { front: "SQL JOIN-Operationen", back: "**INNER JOIN:** Nur übereinstimmende Datensätze beider Tabellen. **LEFT JOIN:** Alle Datensätze der linken + passende der rechten Tabelle. **RIGHT JOIN:** Alle rechten + passende linke. **FULL OUTER JOIN:** Alle Datensätze beider Tabellen. **IHK-Syntax:** SELECT * FROM tabelle1 JOIN tabelle2 ON tabelle1.id = tabelle2.fk_id. **Performance:** Indizierte Spalten für JOIN-Bedingungen verwenden. **Praxistipp:** INNER JOIN ist am häufigsten und schnellsten." },
        { front: "ACID-Eigenschaften im Detail", back: "**Atomicity (Atomarität):** Transaktion ganz oder gar nicht ausführen - bei Fehler komplettes Rollback. **Consistency (Konsistenz):** Datenbank bleibt in gültigem Zustand, alle Constraints werden eingehalten. **Isolation (Isolation):** Parallele Transaktionen beeinflussen sich nicht (verschiedene Isolation Levels). **Durability (Dauerhaftigkeit):** Committed Änderungen überstehen Systemausfälle. **IHK-Bedeutung:** Grundlage für zuverlässige Geschäftsanwendungen, Datenintegrität in Multi-User-Umgebungen." },
        { front: "Datenbank-Indizes", back: "**Definition:** Zusätzliche Datenstruktur zur Beschleunigung von Abfragen. **Arten:** B-Tree (Standard), Hash, Bitmap. **IHK-Vorteile:** Schnellere SELECT, WHERE, ORDER BY, JOIN-Operationen. **Nachteile:** Zusätzlicher Speicher, langsamere INSERT/UPDATE/DELETE. **Automatisch:** Auf Primärschlüssel und UNIQUE Constraints. **Praxisregel:** Auf häufig abgefragte und JOIN-Spalten setzen. **Performance:** Kann Abfragen von Sekunden auf Millisekunden beschleunigen." },
        { front: "Datenbank-Transaktionen", back: "**Definition:** Logische Arbeitseinheit von mehreren DB-Operationen. **Befehle:** BEGIN/START TRANSACTION, COMMIT (erfolgreich speichern), ROLLBACK (rückgängig machen). **Isolation Levels:** READ UNCOMMITTED (Dirty Read möglich), READ COMMITTED (Standard), REPEATABLE READ, SERIALIZABLE (strengstes). **IHK-Anwendung:** Geldüberweisung (Abbuchung + Gutschrift atomisch), Bestellverarbeitung. **Deadlock:** Gegenseitige Blockierung - DB löst durch Abbruch einer Transaktion." },
        { front: "Stored Procedures und Functions", back: "**Stored Procedure:** Vorkompilierter SQL-Code auf DB-Server, kann Parameter haben, kein Rückgabewert erforderlich. **Function:** Muss Wert zurückgeben, in SQL-Anweisungen verwendbar. **IHK-Vorteile:** Performance (vorkompiliert), Sicherheit (SQL-Injection-Schutz), Zentralisierung von Geschäftslogik. **Nachteile:** DB-spezifisch, schwerer zu versionieren. **Beispiel:** Stored Procedure für komplexe Bestellverarbeitung mit mehreren Tabellen-Updates." },
        { front: "NoSQL vs. Relationale Datenbanken", back: "**Relationale DB:** Strukturierte Daten, ACID-Transaktionen, SQL-Standard, vertikale Skalierung. **NoSQL-Arten:** Document (MongoDB), Key-Value (Redis), Column-Family (Cassandra), Graph (Neo4j). **NoSQL-Vorteile:** Horizontale Skalierung, flexible Schemas, bessere Performance bei großen Datenmengen. **IHK-Entscheidungskriterien:** Datenkonsistenz vs. Performance, strukturierte vs. unstrukturierte Daten, Transaktionsanforderungen. **Moderne Ansätze:** Polyglot Persistence - verschiedene DBs für verschiedene Anforderungen." }
      ]
    },
    // ... Weitere Module für Datenbanken
  ],

  // Erweitere alle anderen Kategorien ähnlich detailliert...
