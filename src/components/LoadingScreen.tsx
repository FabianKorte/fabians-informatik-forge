...
export const LoadingScreen = ({ 
  ...
  const [phase, setPhase] = useState<"logo" | "microchip" | "fadeout">("logo");

  useEffect(() => {
    // Phase 1: Logo intro (scale in)
    const logoTimer = setTimeout(() => {
      setPhase("microchip");
    }, 1500);

    // Phase 2: Microchip animation
    const microchipTimer = setTimeout(() => {
      setPhase("fadeout");
    }, minDuration);
  ...
  return (
    <div className={`loading-screen loading-screen--${phase}`}>
      ...
      <div className="logo-intro">
        <img src={logo} alt="IT Azubi Logo" className="logo-intro__image" />
      </div>
